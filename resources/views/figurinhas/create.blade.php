    ...
    <div class="container">
        <div class="header">
            ...
            <div class="breadcrumb">
                <a href="{{ route('figurinhas.index') }}">Catálogo</a> / Adicionar
            </div>
        </div>
        ...
        <form action="{{ route('figurinhas.store') }}" method="POST" enctype="multipart/form-data" id="formFigurinha">
            @csrf

            <div class="form-group {{ $errors->has('nome') ? 'error' : '' }}">
                <label for="nome">Nome <span class="required">*</span></label>
                <input type="text" id="nome" name="nome" value="{{ old('nome') }}" placeholder="Ex: Cristiano Ronaldo"
                    required>
                @error('nome')
            ...
            <div class="form-group {{ $errors->has('pais') ? 'error' : '' }}">
                ...
                <input type="text" id="pais" name="pais" value="{{ old('pais') }}" placeholder="Ex: Portugal" required>
                @error('pais')
                    <div class="error-message">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group {{ $errors->has('numero') ? 'error' : '' }}">
                ...
                <input type="number" id="numero" name="numero" value="{{ old('numero') }}" placeholder="Ex: 1" min="1"
                    required>
            ...
            <div class="form-group {{ $errors->has('time') ? 'error' : '' }}">
                ...
                <input type="text" id="time" name="time" value="{{ old('time') }}" placeholder="Ex: Manchester United"
                    required>
                @error('time')
                    <div class="error-message">{{ $message }}</div>
                @enderror
            ...
            <div class="form-group {{ $errors->has('imagem') ? 'error' : '' }}">
                ...
                <div class="help-text">Formatos aceitos: JPEG, PNG, JPG, GIF (máx. 2MB)</div>
                @error('imagem')
                    <div class="error-message">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-actions">
                <button type="submit" class="btn-primary">Salvar Figurinha</button>
